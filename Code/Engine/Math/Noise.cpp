...
float Compute1dFractalNoise( float position, float scale, unsigned int numOctaves, float octavePersistence, float octaveScale, bool renormalize, unsigned int seed )
{
	...
	for( unsigned int octaveNum = 0; octaveNum < numOctaves; ++ octaveNum )
	{
		...
		float weightEast = MathUtils::EasingFunctions::SmoothStep3( distanceFromWest ); // Gives rounder (nonlinear) results
	...
	if( renormalize && totalAmplitude > 0.f )
	{
		...
		totalNoise = MathUtils::EasingFunctions::SmoothStep3( totalNoise );		// Push towards extents (octaves pull us away)
...
float Compute2dFractalNoise( float posX, float posY, float scale, unsigned int numOctaves, float octavePersistence, float octaveScale, bool renormalize, unsigned int seed )
{
	...
	for( unsigned int octaveNum = 0; octaveNum < numOctaves; ++ octaveNum )
	{
		...
		float weightEast  = MathUtils::EasingFunctions::SmoothStep3( displacementFromMins.x );
		float weightNorth = MathUtils::EasingFunctions::SmoothStep3( displacementFromMins.y );
		float weightWest  = 1.f - weightEast;
...
float Compute3dFractalNoise( float posX, float posY, float posZ, float scale, unsigned int numOctaves, float octavePersistence, float octaveScale, bool renormalize, unsigned int seed )
{
	...
	for( unsigned int octaveNum = 0; octaveNum < numOctaves; ++ octaveNum )
	{
		...
		float weightEast  = MathUtils::EasingFunctions::SmoothStep3( displacementFromMins.x );
		float weightNorth = MathUtils::EasingFunctions::SmoothStep3( displacementFromMins.y );
		float weightAbove = MathUtils::EasingFunctions::SmoothStep3( displacementFromMins.z );
		float weightWest  = 1.f - weightEast;
		float weightSouth = 1.f - weightNorth;
...
float Compute4dFractalNoise( float posX, float posY, float posZ, float posT, float scale, unsigned int numOctaves, float octavePersistence, float octaveScale, bool renormalize, unsigned int seed )
{
	...
	for( unsigned int octaveNum = 0; octaveNum < numOctaves; ++ octaveNum )
	{
		...
		float weightNorth	= MathUtils::EasingFunctions::SmoothStep3( displacementFromMins.y );
		float weightAbove	= MathUtils::EasingFunctions::SmoothStep3( displacementFromMins.z );
		float weightAfter	= MathUtils::EasingFunctions::SmoothStep3( displacementFromMins.w );
		float weightWest	= 1.f - weightEast;
		float weightSouth	= 1.f - weightNorth;
		float weightBelow	= 1.f - weightAbove;
...
float Compute1dPerlinNoise( float position, float scale, unsigned int numOctaves, float octavePersistence, float octaveScale, bool renormalize, unsigned int seed )
{
	...
	for( unsigned int octaveNum = 0; octaveNum < numOctaves; ++ octaveNum )
	{
		...
		float weightEast = MathUtils::EasingFunctions::SmoothStep3( displacementFromWest );
		float weightWest = 1.f - weightEast;
...
float Compute2dPerlinNoise( float posX, float posY, float scale, unsigned int numOctaves, float octavePersistence, float octaveScale, bool renormalize, unsigned int seed )
{
	...
	for( unsigned int octaveNum = 0; octaveNum < numOctaves; ++ octaveNum )
	{
		...
		float dotSouthWest = MathUtils::DotProduct( gradientSW, displacementFromSW );
		float dotSouthEast = MathUtils::DotProduct( gradientSE, displacementFromSE );
		float dotNorthWest = MathUtils::DotProduct( gradientNW, displacementFromNW );
		float dotNorthEast = MathUtils::DotProduct( gradientNE, displacementFromNE );

		// Do a smoothed (nonlinear) weighted average of dot results
		float weightEast = MathUtils::EasingFunctions::SmoothStep3( displacementFromSW.x );
		float weightNorth = MathUtils::EasingFunctions::SmoothStep3( displacementFromSW.y );
		float weightWest = 1.f - weightEast;
		float weightSouth = 1.f - weightNorth;
...
float Compute3dPerlinNoise( float posX, float posY, float posZ, float scale, unsigned int numOctaves, float octavePersistence, float octaveScale, bool renormalize, unsigned int seed )
{
	...
	{
		Vector3( +MathUtils::M_SQRT3_3, +MathUtils::M_SQRT3_3, +MathUtils::M_SQRT3_3 ), // Normalized unit 3D vectors
		Vector3( -MathUtils::M_SQRT3_3, +MathUtils::M_SQRT3_3, +MathUtils::M_SQRT3_3 ), //  pointing toward cube
		Vector3( +MathUtils::M_SQRT3_3, -MathUtils::M_SQRT3_3, +MathUtils::M_SQRT3_3 ), //  corners, so components
		Vector3( -MathUtils::M_SQRT3_3, -MathUtils::M_SQRT3_3, +MathUtils::M_SQRT3_3 ), //  are all sqrt(3)/3, i.e.
		Vector3( +MathUtils::M_SQRT3_3, +MathUtils::M_SQRT3_3, -MathUtils::M_SQRT3_3 ), // 0.5773502691896257645091f.
		Vector3( -MathUtils::M_SQRT3_3, +MathUtils::M_SQRT3_3, -MathUtils::M_SQRT3_3 ), // These are slightly better
		Vector3( +MathUtils::M_SQRT3_3, -MathUtils::M_SQRT3_3, -MathUtils::M_SQRT3_3 ), // than axes (1,0,0) and much
		Vector3( -MathUtils::M_SQRT3_3, -MathUtils::M_SQRT3_3, -MathUtils::M_SQRT3_3 )  // faster than edges (1,1,0).
	};
	...
	for( unsigned int octaveNum = 0; octaveNum < numOctaves; ++ octaveNum )
	{
		...
		float dotBelowSW = MathUtils::DotProduct( gradientBelowSW, displacementFromBelowSW );
		float dotBelowSE = MathUtils::DotProduct( gradientBelowSE, displacementFromBelowSE );
		float dotBelowNW = MathUtils::DotProduct( gradientBelowNW, displacementFromBelowNW );
		float dotBelowNE = MathUtils::DotProduct( gradientBelowNE, displacementFromBelowNE );
		float dotAboveSW = MathUtils::DotProduct( gradientAboveSW, displacementFromAboveSW );
		float dotAboveSE = MathUtils::DotProduct( gradientAboveSE, displacementFromAboveSE );
		float dotAboveNW = MathUtils::DotProduct( gradientAboveNW, displacementFromAboveNW );
		float dotAboveNE = MathUtils::DotProduct( gradientAboveNE, displacementFromAboveNE );

		// Do a smoothed (nonlinear) weighted average of dot results
		float weightEast  = MathUtils::EasingFunctions::SmoothStep3( displacementFromBelowSW.x );
		float weightNorth = MathUtils::EasingFunctions::SmoothStep3( displacementFromBelowSW.y );
		float weightAbove = MathUtils::EasingFunctions::SmoothStep3( displacementFromBelowSW.z );
		float weightWest  = 1.f - weightEast;
		float weightSouth = 1.f - weightNorth;
		float weightBelow = 1.f - weightAbove;
...
float Compute4dPerlinNoise( float posX, float posY, float posZ, float posT, float scale, unsigned int numOctaves, float octavePersistence, float octaveScale, bool renormalize, unsigned int seed )
{
	...
	for( unsigned int octaveNum = 0; octaveNum < numOctaves; ++ octaveNum )
	{
		...
		float dotBeforeBSW = MathUtils::DotProduct( gradientBeforeBSW, displacementFromBeforeBSW );
		float dotBeforeBSE = MathUtils::DotProduct( gradientBeforeBSE, displacementFromBeforeBSE );
		float dotBeforeBNW = MathUtils::DotProduct( gradientBeforeBNW, displacementFromBeforeBNW );
		float dotBeforeBNE = MathUtils::DotProduct( gradientBeforeBNE, displacementFromBeforeBNE );
		float dotBeforeASW = MathUtils::DotProduct( gradientBeforeASW, displacementFromBeforeASW );
		float dotBeforeASE = MathUtils::DotProduct( gradientBeforeASE, displacementFromBeforeASE );
		float dotBeforeANW = MathUtils::DotProduct( gradientBeforeANW, displacementFromBeforeANW );
		float dotBeforeANE = MathUtils::DotProduct( gradientBeforeANE, displacementFromBeforeANE );
		float dotAfterBSW =  MathUtils::DotProduct( gradientAfterBSW, displacementFromAfterBSW );
		float dotAfterBSE =  MathUtils::DotProduct( gradientAfterBSE, displacementFromAfterBSE );
		float dotAfterBNW =  MathUtils::DotProduct( gradientAfterBNW, displacementFromAfterBNW );
		float dotAfterBNE =  MathUtils::DotProduct( gradientAfterBNE, displacementFromAfterBNE );
		float dotAfterASW =  MathUtils::DotProduct( gradientAfterASW, displacementFromAfterASW );
		float dotAfterASE =  MathUtils::DotProduct( gradientAfterASE, displacementFromAfterASE );
		float dotAfterANW =  MathUtils::DotProduct( gradientAfterANW, displacementFromAfterANW );
		float dotAfterANE =  MathUtils::DotProduct( gradientAfterANE, displacementFromAfterANE );

		// Do a smoothed (nonlinear) weighted average of dot results
		float weightEast  = MathUtils::EasingFunctions::SmoothStep3( displacementFromBeforeBSW.x );
		float weightNorth = MathUtils::EasingFunctions::SmoothStep3( displacementFromBeforeBSW.y );
		float weightAbove = MathUtils::EasingFunctions::SmoothStep3( displacementFromBeforeBSW.z );
		float weightAfter = MathUtils::EasingFunctions::SmoothStep3( displacementFromBeforeBSW.w );
		float weightWest   = 1.f - weightEast;
		float weightSouth  = 1.f - weightNorth;
		float weightBelow  = 1.f - weightAbove;
		float weightBefore = 1.f - weightAfter;
