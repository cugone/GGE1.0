...
ReadWriteBuffer::ReadWriteBuffer(RHIDevice* owner, const void* data, unsigned int element_size, unsigned int element_count, const BufferUsage& usage, const BufferBindUsage& bindFlag)
    : IBuffer()
    ...
    , buffer_size(elem_count * elem_size)
{
    ...
    vb_desc.CPUAccessFlags = CPUAccessFlagFromUsage(BufferUsage::STAGING);
...
void ReadWriteBuffer::Update(RHIDeviceContext* context, const void* buffer) {
    D3D11_MAPPED_SUBRESOURCE resource;
    ID3D11DeviceContext* dx_context = context->GetDxContext();

    HRESULT hr = dx_context->Map(_dx_buffer, 0, D3D11_MAP_READ_WRITE, 0u, &resource);
